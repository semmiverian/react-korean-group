...
export default function FindGroup() {
  ...
  async function fetchMember(group) {
    try {
    ...
    <div className="w-4/5 mx-auto">
      <form onSubmit={findGroup} action="" className="my-5">
        <div className="flex w-full mb-6">
          ...
          <button type="submit" className="bg-blue text-white font-semibold rounded-r h-11 w-1/5 ml-auto">
            <span>Find Group</span>
          </button>
        </div>
        {error && <span className="text-red font-normal text-sm">{JSON.stringify(error)}</span>}
      </form>
